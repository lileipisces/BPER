...
rec = BPER(train_tuple_list, len(user2index), len(item2index), len(exp2index), args.learning_rate, args.dimension, args.reg_rate)
...
if args.mu_on_user == -1:
    for mu in np.arange(0, 1.1, 0.1):
        ...
        test_tuple_predict = rec.get_prediction_exp(args.top_k, test_tuple_list, mu)
    ...
    test_tuple_predict = rec.get_prediction_exp(args.top_k, test_tuple_list, args.mu_on_user)
    ndcg, precision, recall, f1 = evaluate_exp(test_tuple_list, test_tuple_predict)
    print(get_now_time() + 'NDCG on test set: {}'.format(ndcg))
    print(get_now_time() + 'Precision on test set: {}'.format(precision))
    print(get_now_time() + 'Recall on test set: {}'.format(recall))
